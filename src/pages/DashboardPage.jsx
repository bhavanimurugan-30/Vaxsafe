    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge variant={stats.activeBreaches[0].status?.toLowerCase() || 'critical'} size="sm">
                    {stats.activeBreaches[0].status || 'CRITICAL'}
                  </Badge>
                </div>
                <p className="text-xs text-rose-800 mt-1">
                  Batch: <strong>{stats.activeBreaches[0].batchId || stats.activeBreaches[0].affectedBatchId}</strong> ({stats.activeBreaches[0].vaccineName || stats.activeBreaches[0].affectedVaccineName}) • Reading: <strong>{stats.activeBreaches[0].temperature}°C</strong> • Certified Range: <strong>{stats.activeBreaches[0].allowedRange || 'Per-batch specification'}</strong>
                  {stats.activeBreaches[0].remainingMinutes !== undefined && (
                    <span> • Remaining: <strong>{stats.activeBreaches[0].remainingMinutes <= 0 ? '0 min (Exceeded)' : `${stats.activeBreaches[0].remainingMinutes} min`}</strong></span>
                  )}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <Button
                ...
                size="sm"
                icon={Route}
                onClick={() => handleRerouteFromAlert(stats.activeBreaches[0].batchId || stats.activeBreaches[0].affectedBatchId)}
              >
                Initiate Reroute
              </Button>
            </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <div className={`p-4 rounded-xl border shadow-sm ${
          ...
          <div className="mt-1">
            <Badge
              variant={stats.tempStatus.toLowerCase()}
              size="sm"
            >
              {stats.tempStatus}
            </Badge>
